...
/**
 * Class WP_EXT_Event_User_Role
 * ------------------------------------------------------------------------------------------------------------------ */

class WP_EXT_Event_User_Role extends WP_EXT_Event {
	...
	public function install() {
		add_role( $this->cap_type[0], esc_html__( 'PT: Event', 'wp-ext-' . $this->domain_ID ), [
			'read'         => true,
			'upload_files' => true,
		] );
	}
 ...
 *
 * @return WP_EXT_Event_User_Role
 * ------------------------------------------------------------------------------------------------------------------ */

function WP_EXT_Event_User_Role() {
	static $object;

	if ( null == $object ) {
		$object = new WP_EXT_Event_User_Role;
	}
...
register_activation_hook( plugin_dir_path( __DIR__ ) . 'plugin.php', [ WP_EXT_Event_User_Role(), 'install' ] );

/**
 * Uninstall plugin.
 * ------------------------------------------------------------------------------------------------------------------ */

register_deactivation_hook( plugin_dir_path( __DIR__ ) . 'plugin.php', [ WP_EXT_Event_User_Role(), 'uninstall' ] );

/**
 * Initialize the object on `plugins_loaded`.
 * ------------------------------------------------------------------------------------------------------------------ */

add_action( 'plugins_loaded', [ WP_EXT_Event_User_Role(), 'run' ] );
